...
export class EntitySheetHelper {
  static getAttributeData (data) {
    ...
    for (const key of groupKeys) {
      const group = data.data.attributes[key] || {}
      ...
      Object.keys(group).sort((a, b) => a.localeCompare(b)).forEach(
        attr => {
          // Avoid errors if this is an invalid group.
          if (typeof group[attr] !== 'object' || !group[attr]) return
          // For each attribute, determine whether it's a checkbox
          // or resource, and then add it to the group's attributes
          // list.
    ...
    Object.keys(data.data.attributes).filter(
      a => !groupKeys.includes(a)).sort((a, b) =>
      a.localeCompare(b)).forEach(key => {
      data.data.ungroupedAttributes[key] =
  ...
  static onSubmit (event) {
    // Closing the form/sheet will also trigger a submit,
    // so only evaluate if this is an event.
    if (event.currentTarget) {
      ...
      let attr = false
      // If this is the attribute key, we need to make a note
      // of it so that we can restore focus when its recreated.
      ...
      // Return the attribute key if set, or true to confirm
      // the submission should be triggered.
      return attr || true
    }
  }
  ...
  /**
     * Listen for click events on an attribute control to modify
     * the composition of attributes in the sheet
  ...
  static onAttributeRoll (event) {
    ...
    const label = button.closest('.attribute')
    ...
    let formula = button.closest('.attribute')
    ...
    if (formula) {
      ...
      if (formula.includes('@item.') && this.item) {
        const itemName = this.item.name.slugify({ strict: true })
      ...
      formula = EntitySheetHelper.replaceData(
        formula, rollData, { missing: null })
  ...
  static getAttributeHtml (items, index, group = false) {
    ...
    // (including whether or not they need a group key).
  ...
  static validateGroup (groupName, entity) {
    const groups = Object.keys(entity.object.data.data.groups)
    const attributes = Object.keys(entity.object.data.data.attributes).filter(a => !groups.includes(a))
  ...
  static async createAttribute (event, app) {
    ...
    if (group) {
      ...
      if (!dtype) {
        const lastAttr = document.querySelector('.attributes > .attributes-group .attribute:last-child .attribute-dtype')?.value
  ...
  static updateAttributes (formData, entity) {
    ...
    const attributes = Object.values(formAttrs).reduce((obj, v) => {
      ...
      if (!v.key) {
        ...
        attrs.forEach(attrKey => {
          ...
          const k = v[attrKey].key ? v[attrKey].key.trim() : attrKey.trim()
    ...
    for (const k of Object.keys(entity.object.data.data.attributes)) {
    ...
    for (const group of groupKeys) {
      if (entity.object.data.data.attributes[group]) {
        for (const k of Object.keys(entity.object.data.data.attributes[group])) {
  ...
  static updateGroups (formData, entity) {
    ...
    for (const k of Object.keys(entity.object.data.data.groups)) {
  ...
  static replaceData (formula, data, { missing = null, depth = 1 } = {}) {
    ...
    const dataRgx = /@([a-z.0-9_-]+)/gi
    const rollFormula = formula.replace(dataRgx, (match, term) => {
      const value = getProperty(data, term)
