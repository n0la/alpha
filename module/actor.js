...
export class SimpleActor extends Actor {
  ...
  getRollData () {
    ...
    // Evaluate formula attributes after all other attributes have
    // been handled, including items.
  ...
  _applyItems (data, itemAttributes, shorthand) {
    ...
    data.items = this.data.items.reduce((obj, i) => {
      const key = i.name.slugify({ strict: true })
      const itemData = duplicate(i.data)
    ...
    for (let k of itemAttributes) {
      ...
      if (shorthand) {
        ...
        if (data.items[item][k][gk]) {
          ...
          data.items[item][k][gk] =
                        EntitySheetHelper.replaceData(
                          formula.replace('@item.', `@items.${item}.`),
                          data, { missing: '0' })
  ...
  _applyFormulaReplacements (data, formulaAttributes, shorthand) {
    ...
    for (let k of formulaAttributes) {
      ...
      if (data.attributes[k]?.value) {
        data.attributes[k].value = EntitySheetHelper.replaceData(
          data.attributes[k].value, data, { missing: '0' })
        // TODO: Replace with:
